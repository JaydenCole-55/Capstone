...
def plot_green(data):
    ...
    for i in range(GRID_SIZE_Y):
        ...
        for j in range(GRID_SIZE_X):
            if (data[i][j][Z] == 0):
                ...
                magArr.append(abs(data[i][j][Z])*8)
    ...
    # Plot
    plt.quiver(qX, qY, scale=5, scale_units="inches", pivot="mid")
...
def create_diff_map():
    #######################################
    #
    # Creates a heat map of the diff grid
    #
    ######################################
    ...
    for i in range(GRID_SIZE_Y):
        ...
        for j in range(GRID_SIZE_X):
            ...
            if i == 9 and j == 13:
                ...
                magArr.append(float(np.asarray(file_handle.readline()[:-1].split(','))[3]))
